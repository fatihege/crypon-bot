...
module.exports = {const { botName } = require("../config.json");
const db = require("wio.db");
...
module.exports = {
	...
	permissions: 'KICK_MEMBERS',
	async run(message, args, client) {
        if (message.mentions.users.first().bot && message.mentions.users.first().username == botName) {
            return message.reply(`Hey sen! Beni bu komutla **atamazsın!**`)
                .then(msg => {
                    msg.delete({ timeout: 5000 });
                });
        }
        ...
        if (!args.length || args.length < 2) {
            ...
            return message.reply(`Lütfen gerekli argümanları ver!\nBu komutun kullanımı \`${prefix}${this.name} ${this.usage}\``)
                .then(msg => {
                    msg.delete({ timeout: 5000 });
                });
        }
		...
		if (!message.mentions.members.first()) {
			return message.reply("Lütfen sunucudan atmak istediğin üyeyi etiketle!")
                .then(msg => {
                    msg.delete({ timeout: 5000 });
                });
		}
		...
		if (message.mentions.members.first().hasPermission("ADMINISTRATOR")) {
			return message.reply("Yavaş ol, bir yönetici kendi sunucusundan atılamaz!")
                .then(msg => {
                    msg.delete({ timeout: 5000 });
                });
		}
		...
		try {
            taggedUser.kick();
            message.channel.send(`${taggedUser} sunucudan başarıyla **atıldı!**\n**Sebebi:** ${reason}`);
        } catch (error) {
            if (error) {
                console.error(error)
                message.channel.send(`${taggedUser} sunucudan atılırken bir **hata oluştu...**`);
            }
        }
	}
}
