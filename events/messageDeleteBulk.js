        ...
        if (messages.first().guild) {
					...
					if (await db.fetch("logch_" + message.guild.id)) {
						...
						messages.sort().map(message => {
							if (message.author.bot  || message.channel.name == logch.name) {
					...
					messages.first().channel.send("Sunucu bilgisi alınamadı. Lütfen geliştiricimle iletişime geçin: https://fatihege.github.io/");
				}
    }
}